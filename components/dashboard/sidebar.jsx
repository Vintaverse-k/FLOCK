...
export function DashboardSidebar() {
  ...
  return (
    <div className={cn("bg-white border-r border-gray-200 flex flex-col", collapsed ? "w-16" : "w-64")}>
      {/* Logo */}
            ...
            <nav className="space-y-1">
              {navigation.map((item) => {
                ...
                return (
                  ...
                  >
                    <img src={item.icon} alt={item.name} className="h-5 w-5 mr-3 flex-shrink-0" />
                    {!collapsed && item.name}
                  </Link>
                )
              })}
